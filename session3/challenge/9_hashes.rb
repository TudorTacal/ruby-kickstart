...
def shared(a, b)
    hash = Hash.new {|this_hash, key| this_hash[key] = [a.include?(key)||nil,b.include?(key)||nil]}
    a.each{|x| hash[x]}
    b.each{|x| hash[x]}
    values = hash.select{|key, value| hash[key] if value == [true,true]}.keys
    result =[hash,values.sort]
